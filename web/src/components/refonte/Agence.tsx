...
function WordReveal({ text }: { text: string }) {
  ...
  useEffect(() => {
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      // Animation coupee : le texte passe en pleine opacite a la frame
      // suivante, jamais dans le corps de l'effet.
      const immediat = requestAnimationFrame(() => setProgress(1))
      return () => cancelAnimationFrame(immediat)
    }
    let raf = 0
    const onScroll = () => {
...
function Stat({
  ...
  useEffect(() => {
    if (!inView) return
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      // Animation coupee : le chiffre final est pose a la frame suivante.
      const immediat = requestAnimationFrame(() => setDisplay(value))
      return () => cancelAnimationFrame(immediat)
    }
