...
- Déploiement : branche `next-payload` en Preview d'abord, puis Root Directory Vercel `web`, build `pnpm run build:vercel`. Détails et commandes dans `web/README.md`, variables dans `web/.env.example`. `app/` reste déployé jusqu'à la bascule, puis sera supprimé.
